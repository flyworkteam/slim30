...
enum _GoalSpeed { slow, medium, fast, veryFast }

class QuestionGoalSpeedView extends StatefulWidget {
  const QuestionGoalSpeedView({super.key});

  @override
  State<QuestionGoalSpeedView> createState() => _QuestionGoalSpeedViewState();
}

class _QuestionGoalSpeedViewState extends State<QuestionGoalSpeedView> {
  _GoalSpeed? _selected;

  static const int _currentStep = 12;
  ...
  Widget build(BuildContext context) {
    ...
    final items = [
      _Item(
        _GoalSpeed.slow,
        'iconsax-trend-down.svg',
        l10n.questionGoalSpeedSlow,
      ),
      _Item(
        _GoalSpeed.medium,
        'iconsax-speedometer.svg',
        l10n.questionGoalSpeedMedium,
      ),
      _Item(
        _GoalSpeed.fast,
        'iconstack.io - (Fire Light).svg',
        l10n.questionGoalSpeedFast,
      ),
      _Item(
        _GoalSpeed.veryFast,
        'iconsax-flash.svg',
        l10n.questionGoalSpeedVeryFast,
      ),
    ];
                        ...
                        children: [
                          for (int col = 0; col < 2; col++) ...[
                            _SpeedCard(
                              ...
                              selected: _selected == items[row * 2 + col].speed,
                              onTap: () => setState(
                                () => _selected = items[row * 2 + col].speed,
                              ),
                            ),
...
class _Item {
  const _Item(this.speed, this.icon, this.label);
  final _GoalSpeed speed;
  final String icon;
  final String label;
}
